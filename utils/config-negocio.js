...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = 'd1d3d70e-6913-4205-9c33-36ebf0473eb1'; // ID de Mis chicas Nails
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Mis chicas Nails';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '52048901';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'ribaltaalexxandra@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Mis chicas Nails!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'mischicasnails';
};
...
console.log('✅ config-negocio.js listo para Mis chicas Nails');
